...
const registerUser = asyncHandler(async (req, res) => {
    //get user detail fromfrontend
    //validation-not emoty
    //check if user exist
    //check for images and avtar
    //upload them in cloudnary, avtar
    //create user object - creat entry in db
    //remove password and refresh token in responce
...
const updateAccountDetails = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, user, "Account details updated success fuly"))
});

const updateUserAvatair = asyncHandler(async (req, res) => {
    const avatarLocalPath = await req.file?.path

    if (!avatarLocalPath) {
        throw new ApiError(400, "avtair file is messing")
    }

    const avatar = await uploadOnCloudinary(avatarLocalPath)
    if (!avatar.url) {
        throw new ApiError(500, "error while uploading avatair")
    }
    ...
    return res
        .status(200)
        .json(
            new ApiResponse(200, user, "avtar image updated sucessfuly")
        )
})
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(
            new ApiResponse(200, user, "cover image updated sucessfuly")
        )
})
